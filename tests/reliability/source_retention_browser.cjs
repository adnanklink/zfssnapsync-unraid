...
(async()=>{
 ...
 try{
  ...
  await page.getByLabel('Source snapshots',{exact:true}).selectOption('count');
  ...
  assert.match(await page.locator('.source-review-status').textContent(),/4 currently eligible; 3 protected; 2 unmanaged/);
  assert.equal(await page.locator('.source-review-list li').count(),50);
  assert(await page.locator('.source-review-list').evaluate(el=>el.scrollWidth<=el.clientWidth+1),'Long review paths overflow');
  await page.getByRole('button',{name:'Next',exact:true}).click();await page.waitForFunction(()=>document.querySelector('.source-review-results p')?.textContent.includes('51–70'));
  await page.getByRole('button',{name:'Use this retention policy',exact:true}).click();
  assert.equal(await page.locator('[name="job_source_review[0]"]').inputValue(),'a'.repeat(48));
  await page.locator('#finish-job-edit').click();
  await page.locator('[data-restore-tuning]').click();assert.equal(await page.locator('[name="job_source_keep[0]"]').inputValue(),'3','Tuning defaults changed cleanup authorization');
  await page.locator('#save_send_btn').click();await page.waitForTimeout(250);assert.equal(save.get('job_source_keep[0]'),'3');assert.equal(save.get('job_source_review[0]'),'a'.repeat(48));
  assert.equal(await page.locator('[name="job_source_review[0]"]').inputValue(),'','Saved review token remained stale');
  await page.getByRole('button',{name:'Edit',exact:true}).click();slow=true;
  await page.getByRole('button',{name:'Review source snapshots',exact:true}).click();
  await page.getByLabel('Source checkpoint count',{exact:true}).fill('4');await page.waitForTimeout(900);
  ...
  assert.equal(await page.locator('[name="job_source_review[0]"]').inputValue(),'');
  await page.keyboard.press('Escape');
  await page.getByRole('button',{name:'Edit',exact:true}).click();assert.equal(await page.getByLabel('Source checkpoint count',{exact:true}).inputValue(),'3','Cancel edit lost count');

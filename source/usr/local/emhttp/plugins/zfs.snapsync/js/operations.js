(() => {
  ...
  let snapshot=null, selected=null, busy=false, logPoll=null,detailLogPoll=null;
  ...
  function detail(op) {
    ...
    const markup=problem+'<dl>'+entries.map(([key,value])=>'<dt>'+escape(key)+'</dt><dd>'+escape(value)+'</dd>').join('')+'</dl>'+(Number.isFinite(op.progress)?'<label>Reported progress<progress max="100" value="'+Math.max(0,Math.min(100,op.progress))+'"></progress>'+escape(op.progress)+'%</label>':'')+(op.problem?'':'<p class="ui-notice">'+escape(op.message||'No additional message recorded.')+'</p>')+(op.recoveryRequired?'<p class="error">Recovery requires review before another transfer. Use the recovery action below when available.</p>':'')+'<a href="'+escape(ZfsasUI.workflowUrl(op.url))+'">Open workflow →</a>';
    ...
    if(actions.dataset.fingerprint!==fingerprint) {
      ...
      if(op.coordinator || op.id.startsWith('coordinator:')){const log=document.createElement('button');log.type='button';log.textContent='Show job log';log.addEventListener('click',()=>showDetailLog(op));actions.append(log);}
  ...
  function open(id,trigger) {const op=snapshot.operations.find(item=>item.id===id);if(!op)return;if(selected!==id){detailLogPoll?.stop();$('operation-log-panel')?.remove();}selected=id;$('operation-action-message').textContent='';detail(op);ZfsasUI.open($('operation-detail'),trigger);}
  ...
  async function showDetailLog(op,shared=false,offset='latest') {
    ...
    const render=data=>{
      ...
      output.dataset.loaded='1';output.scrollTop=first||bottom?output.scrollHeight:scroll;drawer.scrollTop=drawerScroll;
      ...
      const pages=panel.querySelector('.operation-log-pages');pages.replaceChildren();
      ...
      return snapshot?.operations.some(item=>item.id===op.id&&active(item));
    };
    detailLogPoll=ZfsasRequests.poll('detail-log',()=>ZfsasRequests.request('detail-log',url),render);
  }
  $('operation-detail').addEventListener('close',()=>{detailLogPoll?.stop();detailLogPoll=null;});

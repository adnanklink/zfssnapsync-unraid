...
(() => {
  ...
  if(form) {
    ...
    const advancedHeading=document.createElement('h3');advancedHeading.textContent='Advanced';advancedHeading.hidden=true;$('automation-advanced').querySelector('summary').after(advancedHeading);
  ...
  if($('manager')) {
    ...
    [...cleanupDetails.children].filter(node=>node.tagName!=='SUMMARY').forEach(node=>cleanupPanel.append(node));cleanupDetails.replaceWith(cleanupPanel);
    const takeToggle=button('Take snapshot',()=>togglePanel(takePanel,takeToggle),'btn');
    const cleanupToggle=button('Preview cleanup',()=>togglePanel(cleanupPanel,cleanupToggle),'btn');

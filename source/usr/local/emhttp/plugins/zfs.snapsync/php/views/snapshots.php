...
<label>Snapshot name<input name="search" type="search"></label>
...
<details class="ui-advanced"><summary>More filters</summary><div class="filters"><label>Prefix<input name="prefix"></label><label>Created from (UTC)<input name="from" type="date"></label><label>Through (UTC)<input name="until" type="date"></label>
...
</div><div class="toolbar" aria-label="Quick filters"><span>Quick filters:</span>
<button type="button" data-quick="used">Used = 0 B</button><button type="button" data-quick="written">Written = 0 B</button><button type="button" data-quick="auto">Auto Snapshot</button><button type="button" data-quick="send">Send checkpoints</button><button type="button" data-quick="other">Other snapshots</button></div></details><button type="reset" class="btn-quiet">Clear filters</button></form>
...
<div class="toolbar sticky" id="snapshot-selection-bar" hidden>
<strong id="selected-count">0 selected</strong><button id="select-matching" class="btn-quiet" hidden>Select all matching</button><button id="clear-selection" class="btn-quiet">Clear</button>
<button data-bulk="delete">Delete</button><details class="ui-action-menu"><summary>Protection</summary><button data-bulk="hold">Add plugin hold</button><button data-bulk="release">Release plugin hold</button></details>
...
<th>Protection and pending actions</th><th>Actions</th></tr></thead><tbody id="snapshots"></tbody></table></div>
...
<dialog id="snapshot-transfer" aria-labelledby="snapshot-transfer-title"><form id="snapshot-transfer-form">
<div class="ui-dialog-header"><h2 id="snapshot-transfer-title">Send snapshot</h2><button type="button" data-close-dialog>Cancel</button></div>
<p id="snapshot-transfer-source"></p><p id="snapshot-transfer-help"></p>
<label for="snapshot-transfer-destination">Destination dataset</label><input id="snapshot-transfer-destination" required placeholder="backup/replicas/example" pattern="[^@\s]+/[^@\s]+">

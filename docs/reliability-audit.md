...
## Job diagnostics and reviewed local recovery (unreleased)

Operation summaries prioritize failed tasks and separate dependent work that did not run. Native receive stderr is captured as a sanitized 4 KiB tail per attempt, with its exit code and timestamps. The latest diagnostic remains attached to a task when subsequent validation reports an interrupted receive. The read-only job-detail endpoint returns at most 200 events/128 KiB and never substitutes a shared log. Logs and errors are rendered as text. Earlier releases' discarded diagnostics cannot be reconstructed.

Recovery review runs as coordinator-owned, read-only preparation work, with fixed membership and one receiver enumeration for the review. Exact source, base and receiver references remain protected through a five-minute approval lease. Approval creates an idempotent, revision-bound run containing only the reviewed transfer/verification graph; it creates no snapshots or deletion authority. Resume selection comes from the validated token's original snapshot identity, not the latest failed Run Now snapshot. Changed GUIDs, bases, tokens, revisions and missing source snapshots fail closed. Successful verification resolves only matching recovery obligations; original failed history and unresolved members remain.

Run Now routes retained unresolved failures to review, and scheduled preparation checks interrupted receivers before creating source snapshots. Review is also available from a saved local configuration after RAM loss; discovery does not recreate approval or invent missing snapshot intent. Normal schedule timing and persistent pauses remain intact. There is no automatic receive abort, destination destruction or forced rollback.

Runtime storage remains the existing `/tmp/zfs-snapsync-coordinator` journal and attempt-input directory, with ownership/locks under `/var/run`. The diagnostic and review GET endpoints do not start the coordinator, inspect ZFS, change ownership, or wake the executor's admission loop. Reviews, errors and execution approval disappear on reboot. Existing terminal-history bounds apply; active and unresolved work retain required reference evidence.

Coverage added: `operation_diagnostics.php` (failure priority, attempt isolation/history, redaction and bounded pagination), `replication_recovery.php` (snapshot A versus B, GUID validation, approval expiry, reference leases and idempotence), `recovery_browser.cjs` (job/shared scope, actionable messages, scroll stability, partial review, wrapping and stale responses), and `replication_recovery_zfs.sh` (real interrupted incremental/full receives, read-only flash, production daemon/workers and endpoints, restart, repeated Run Now, completed child preservation, and actual quota errors retained across later recovery validation). These are disposable test fixtures, never production pool recovery commands.

Verification: the instrumented real-ZFS recovery fixture recorded 207,044 syscall lines with zero `/boot` write or metadata-change attempts. Stage-one, reliability, endpoint and browser suites passed, as did PHP parsing, ShellCheck and temporary package inventory verification. Real-ZFS recovery covered both interrupted full and incremental receives and retained a real quota error across the subsequent interrupted-receive failure.

## Receive policy verification (2026.09.23.05)

Backup receives now request `readonly=on`; native workers set existing receivers under dataset gates and verify the property at completion. Explicit Snapshot Manager Restore captures `purpose=restore`, admits only a new target, and requests `readonly=off`. The purpose survives plan publication, receipt lookup and Retry. No automatic mount or rollback is added.

Validation passed: reliability suite, Stage 1 transport suite, Chromium Snapshot Manager including distinct Send/Restore submissions, native admission/receipt/retry tests, PHP lint, Bash parsing, ShellCheck error checks, and installation compatibility fixtures. Disposable real-ZFS full/incremental/resume and scheduled recursive graphs passed. A direct Restore endpoint fixture verified writable restoration beneath a read-only parent, retained backup protection, refusal of an existing target, and policy correction on already-received verification. The general legacy `batch_endpoints.php` fixture remains incompatible with current coordinator setup/mocked inventory (601 skipped instead of expected partial completion); it is not claimed as passing for this release.
